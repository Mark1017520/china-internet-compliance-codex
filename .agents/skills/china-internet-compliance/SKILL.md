...
每次审查按以下步骤执行。V1.0.1 起，必须先使用 `references/INPUT_FACT_SCHEMA.md` 抽取事实，并在涉及明确法规判断时调用 `references/ARTICLE_LEVEL_RULES.md`。
